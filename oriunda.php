...
<head>
	...
	<title>Mantenimiento || Oriunda</title>
	<!--Bootstrap Styles-->
...
<body>
	...
	</script><?php }elseif(isset($_COOKIE['_rac']) && base64_decode($_COOKIE['_rac'])==='terranorte') {?>
	<script>
		window.location.href = "index.php"
	</script><?php }else {?>
	<div class="container">
			  	...
			  	<div class="border-right border-dark">
			  		<div class="row">
			  			<div class="cus-border col-md-12">
			  				...
			  				<div class="scrollbar-mini scrollbar-primary">
	...
	<script>
		$(document).ready(function() { 
			$.post('anularArticulos.php',{basedatos:'oriunda'},function(){
					...
					if (!$('#familia button').hasClass('active')) {
						...
						if ($('#tabla tbody tr').hasClass('selected')){
							...
							if (posicion < 1) {
								...
								$.post('posicionarProducto.php',{basedatos:'oriunda',opcion:1,familia:familia,parametros:param},function(){
									limpiarTablas(4)
				...
				$('#crear').on('click',function(){
					let item = "<div style='display:flex;align-items: baseline;'>"
						...
						item += '</div>'
						item += "<div id='listaSegmentos' class='scrollbar-mini scrollbar-primary scr-content'>"
					...
					$('#anuladosVisible').on('click',function(){
						if ($('#anuladosVisible').prop('checked')) {
							...
							$.post('obtenerSegmento.php',{basedatos:'oriunda',opcion:$('#segmentos').val(),anulado:1,orden:1},function(e){
								let anulados = ''
					...
					$('#segmentos').on('change',function(){
						switch($(this).val()){
							...
							$('#lista').empty()
							let segmentos = ''
							$.post('obtenerSegmento.php',{basedatos:'oriunda',opcion:$(this).val(),anulado:0,orden:1},function(e){
								const json = JSON.parse(e)
								$.each(json,function(i,value){
				...
				$('#actualizar').on('click',function(){
					if ($('#descripcion').val() == '') {
						...
						$.post('editarArticulo.php',{basedatos:'oriunda',parametros:parametros},function(e){
							if (e == 'success') {
								swal({
									type: 'success',
				...
				$('#incluir').on('click',function(){
					...
					if ($('#tabla tbody tr').hasClass('selected')){
						...
						if (todo.length == 0 && parte.length > 1) {
							...
							$.post('obtenerSegmento.php',{basedatos:'oriunda',opcion:1,anulado:0,orden:0},function(e){
								...
								$.post('obtenerSegmento.php',{basedatos:'oriunda',opcion:2,anulado:0,orden:0},function(e){
									...
									$.post('obtenerSegmento.php',{basedatos:'oriunda',opcion:3,anulado:0,orden:0},function(e){
										...
										$.post('obtenerSegmento.php',{basedatos:'oriunda',opcion:4,anulado:0,orden:0},function(e){
				...
				$('#asignar').on('click',function(){
					...
					if (todo.length > 0 && parte.length == 0) {
						$.each(todo,function(i,value){
							...
							$.post('asignarArticulos.php',{basedatos:'oriunda',opcion:0,datos:datos},function(e){
								if (e == 'error') {
									swal({
										type: 'error',
						...
						$.post('asignarArticulos.php',{basedatos:'oriunda',opcion:1,datos:datos},function(e){
							if (e == 'error') {
								swal({
									type: 'error',
									title: 'Ocurrio un error mientras se asignaban articulos',
				...
				$('#excel').on('click',function(){
					$.post('obtenerArticulosIncompletos.php',{basedatos:'oriunda'},function(e){
						if (e != '[]') {
							...
							saveAs(new Blob([s2ab(wbout)],{type:"application/octet-stream"}), 'articulos_incompletos_or.xlsx')
						}else {
				...
				$('#ordenar').on('click',function(){
					if ($('#familia button').hasClass('active')) {
						...
						let familia = $('#familia button.active').val()
						let datos = $('#categoria button.active').val().split('@')[0]+'@'+$('#linea button.active').val().split('@')[0]+'@'+familia.split('@')[2]+'@'+familia.split('@')[0]
						$.post('ordenarArticulos.php',{basedatos:'oriunda',dato:datos},function(){
							swal.close()
							limpiarTablas(4)
		...
		function editarDescripcion(id){
			if($('#'+id).is('[readonly]')){
				...
				$.post('editarSegmento.php',{basedatos:'oriunda',parametros:param,segmento:segmento},function(e){
					if (e == 'error') {
						swal({
							type: 'error',
		...
		function anularSegmento(id){
			if ($('#'+id).hasClass('subrayado')) {
				let segmento = $('#segmentos').val()
				$.post('anularHabilitar.php',{basedatos:'oriunda',id:id,segmento:segmento,anular:0},function(e){
					if (e=='error') {
						swal({
							type: 'error',
				...
				swal({
					...
					if (result.value) {
					    let segmento = $('#segmentos').val()
						$.post('anularHabilitar.php',{basedatos:'oriunda',id:id,segmento:segmento,anular:1},function(e){
							if (e=='error') {
								swal({
									type: 'error',
		...
		function guardarNuevoSegmento(){
			...
			if (descripcion == '') {
				...
				$.post('crearSegmento.php',{basedatos:'oriunda',segmento:segmento,id:id,descripcion:descripcion},function(e){
					if (e == 'success') {
						swal({
							type: 'success',
	        	...
	        	if (opcion == 0) {
	        		...
	        		if (x == 13) {
	        			if ($('#editFactor').val() == '') {
							...
							$.post('asignarFactor.php',{basedatos:'oriunda',parametros: param},function(){
								if (e == 'error') {
									swal({
										type: 'error',
		...
		function llenandoCategorias(seleccionado){
			...
			$.post('obtenerCategoria.php',{basedatos:'oriunda'},function(e){
				let item = ""
				if (e != "[]") {
					const json = JSON.parse(e)
		...
		function llenandoLineas(categoria,seleccionado){
			let nombre = seleccionado || 'ninguno'
			$.post('obtenerLineas.php',{basedatos:'oriunda',categoria:categoria},function(e){
				let item = ""
				const json = JSON.parse(e)
				for (let j = 0; j < json.length; j++) {
					if (nombre != 'ninguno' && json[j].descrip == nombre) {
		...
		function llenandoGenericos(linea,seleccionado){
			let nombre = seleccionado || 'ninguno'
			$.post('obtenerGenericos.php',{basedatos:'oriunda',linea:linea},function(e){
				let item = ""
				const json = JSON.parse(e)
				for (let j = 0; j < json.length; j++) {
					if (nombre != 'ninguno' && json[j].descrip == nombre) {
		...
		function llenandoFamilias(generico,seleccionado){
			...
			let nombre = seleccionado || 'ninguno'
			$.post('obtenerFamilias.php',{basedatos:'oriunda',generico:genericoCompleto},function(e){
				let item = ""
				if (e != "[]") {
					const json = JSON.parse(e)
					for (let j = 0; j < json.length; j++) {
			...
			if (dato == 0) {
				tabla = $('#tabla').DataTable({
					...
					initComplete: function(settings, json) {
						$('body').find('.dataTables_scrollBody').addClass("scrollbar-dataTable-primary")
					}
				})
			}else {
				tabla = $('#tabla').DataTable({
